function automate() {
  ...
  let dragonUpdates = [
    Game.Upgrades["Dragon teddy bear"],
    Game.Upgrades["Dragon claw"],
    Game.Upgrades["Dragon fang"],
    Game.Upgrades["Dragon scale"],
  ];
  let unlockedlist = [];
  pettime = 10000;
  setTimeout(function myFunction() {
    ...
    if (Game.PrestigeUpgrades[83].bought == 1 && Game.dragonLevel >= 8) {
      ...
      if (
        dragonUpdates.filter((itm) => itm.unlocked == 1).length !=
        unlockedlist.length
      ) {
  ...
  setInterval(() => {
    ...
    if (
      Game.dragonLevel == 27 &&
      (Game.dragonAura == 0 || Game.dragonAura2 == 0)
    ) {
      if (!Game.dragonAura) {
    ...
    if (
      Game.Upgrades["Festive biscuit"].unlocked &&
      Game.season !== "christmas" &&
      (Game.cookiesPs + 50 * Game.computedMouseCps) * 20 + Game.cookies >=
        Game.Upgrades["Festive biscuit"].basePrice
    ) {
    ...
    Game.UpgradesInStore.filter((obj) => obj.pool != "toggle").forEach(
      (obj) => {
        if (obj.canBuy() && obj.id !== 69) obj.buy();
        if (obj.canBuy() && obj.id === 69) {
    ...
    getBiasedIndex = function () {
      ...
      I = Math.round(
        rnd * (1 - mix) +
          ((3 * MaxIndex + 5 * (unlockedupg.length - 1)) / 8) * mix,
      );
      console.log(I);
      return I;
    };
  ...
  setInterval(
    () => {
      prevArray = [];
      newArray = Game.PrestigeUpgrades.filter(
        (upg) =>
          !upg.dname.startsWith("Permanent upgrade") &&
          upg.bought == 0 &&
          upg.parents?.every((par) => par.bought != 0),
      ).sort((a, b) => {
        return a.basePrice - b.basePrice;
      });
      reqPrice = newArray.reduce((accumulator, val, index) => {
        if (index < 4) {
          return accumulator + val.basePrice;
        }
      }, 0);
  ...
  setInterval(() => {
    ...
    if (Game.wrinklers.filter((wrk) => wrk.phase > 1).length > 0) {
      Game.wrinklers
        .filter((wrk) => wrk.phase > 1)
        .forEach((wrk) => (wrk.hp = 0));
    }
